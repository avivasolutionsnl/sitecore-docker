Run Sitecore 9.1 XP0 using Docker for Windows.
...
NB. To prefix the Docker images with your repository name change the `RepoImagePrefix`, `XpImagePrefix` build setting parameters.


# Run
Docker compose is used to start up all required services. 
Docker compose files are present for each setup in their respective directories, e.g. `xp`. Use your setup of choice as working directory for all docker-compose commands below.
